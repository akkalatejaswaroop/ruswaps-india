...
export default function ECActPage() {
  ...
  const loadActData = async () => {
    try {
      setLoading(true);
      const res = await fetch("/api/documents/acts?type=ec-act");
      const data = await res.json();
      if (data.success) {
        setActData(data.data);
      }
    } catch (err) {
      console.error("Failed to load EC Act data:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Employee Compensation Act, 1923</h1>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className="bg-gradient-to-r from-secondary to-primary rounded-2xl p-6 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/80 text-sm">
                The Employees Compensation Act, 1923 provides for payment of compensation to workers and their 
                dependents in case of injury and accident occurring during employment.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary dark:bg-gray-700 dark:text-white"
              />
            </div>
          </div>
        </div>
